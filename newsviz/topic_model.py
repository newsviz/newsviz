...
class TopicModelWrapperARTM:
    ...
    def init_model(self, dictionary_path=None):
        ...
        if dictionary_path is None:
            ...
            self.dictionary.save_text(f"{self.dir_path}/dicts/dict_{self.name_dataset}.txt")
        else:
        ...
        self.model.regularizers.add(artm.DecorrelatorPhiRegularizer(name="DecorrelatorPhi", tau=1.5e5))
    ...
    def print_top_words(self):
        ...
        for topic in phi.columns:
            print(topic)
            top_words = phi.sort_values(by=topic, ascending=False)["word"].apply(lambda x: x[1]).values[:20]
            print(top_words)
            print("==" * 5)
    ...
    def save_top_words(self, path, top=20):
        ...
        for topic in phi.columns.drop("word"):
            top_words = phi.sort_values(by=topic, ascending=False)["word"].values[:top]
